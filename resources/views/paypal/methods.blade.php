@if (setting('payment_paypal_status') == 1)
    <li class="list-group-item">
        <input class="magic-radio js_payment_method" type="radio" name="payment_method" id="payment_paypal"
               @if (setting('default_payment_method') == \Tec\Payment\Enums\PaymentMethodEnum::PAYPAL) checked @endif
        ...
        <div class="payment_paypal_wrap payment_collapse_wrap collapse @if (setting('default_payment_method') == \Tec\Payment\Enums\PaymentMethodEnum::PAYPAL) show @endif" style="padding: 15px 0;">
            {!! clean(setting('payment_paypal_description')) !!}

            @php $supportedCurrencies = (new \Tec\Payment\Services\Gateways\PayPalPaymentService)->supportedCurrencyCodes(); @endphp
